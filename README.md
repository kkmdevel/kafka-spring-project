# Kafka를 이용한 실시간 스트리밍 데이터 처리 시스템
...
이 시스템은 웹 환경에서 효율적인 로그 관리와 분석을 가능하게 하며, **마케팅 전략 수립에 큰 도움**을 줄 수 있습니다.
...
웹 로그 데이터를 실시간으로 처리하고 다양한 분석과 데이터 처리를 수행할 수 있는 기능을 제공합니다. 

주요 기능은 다음과 같습니다:
1. **실시간 로그 데이터 수집**
   - Kafka를 통해 웹 로그 데이터를 실시간으로 수집하고 이를 다양한 스트림으로 처리합니다.
   - 이를 통해 실시간 데이터를 효과적으로 관리할 수 있습니다.

2. **데이터 필터링 및 가공**
   - 지정된 조건에 따라 로그 데이터에 특정 컬럼을 추가/제거하여 데이터를 가공할 수 있습니다.
...
4. **이탈 감지**
   - Debezium과 Kafka를 활용하여 사용자 활동에서 이상을 감지하고, 이탈 상황을 실시간으로 모니터링합니다.
...
- 스키마를 포함한 JSON 메시지에서 **ksqlDB가 특정 데이터 필드를 제대로 인식하지 못하는 오류**가 발생했습니다
